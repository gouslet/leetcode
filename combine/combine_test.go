 ...
 * Author: elchn                                                               *
 * -----                                                                       *
 * Last Modified: Sunday, 2022/03/27 , 23:25:45                                *
 * Modified By: elchn                                                          *
 * -----                                                                       *
 * HISTORY:                                                                    *
 * Date      	By	Comments                                                   *
...
func TestCombine3(t *testing.T) {
	...
	for _, test := range tests {
		res := combine3(test.n, test.k)
		sort2DInts(res)
		sort2DInts(test.res)
		if !reflect.DeepEqual(res, test.res) {
			t.Errorf("combine3(%d,%d) = %v,want %v", test.n, test.k, res, test.res)
		}
	}
}
...
func TestCombine4(t *testing.T) {
	...
	for _, test := range tests {
		res := combine4(test.n, test.k)
		sort2DInts(res)
		sort2DInts(test.res)
		if !reflect.DeepEqual(res, test.res) {
			t.Errorf("combine4(%d,%d) = %v,want %v", test.n, test.k, res, test.res)
		}
	}
}
